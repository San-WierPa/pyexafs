...
class PyExafs(object):
    ...
    def clip_data(self, data: np.ndarray, minimum: float = -5, maximum: float = 5) -> np.ndarray:
        """Clip k-data to minimum and maximum to avoid unwanted behaviour in k."""
        return np.clip(data, a_min=minimum, a_max=maximum)


    def load_data(self, measurement_data: np.ndarray, source: str = "SYNCHROTRON",
                  mode: str = "ABSORPTION", processed: str = "RAW", name: Optional[str] = None,
                  plot: bool = True) -> None:
        """
    ...
    def find_e0(self, energy: np.ndarray, mu: np.ndarray, group: Optional[Group] = None,
                take_first: bool = False) -> float:
        """
    ...
    def plot_data(self, data_type: str = 'RAW', show_name: bool = True,
        ...
        minor_ticks_xanes = np.arange(int(np.round(self.data.energy[0], decimals=-1)), int(self.data.energy[-1]), 5)
...
#
    ...
    #            "processed": data_type,},
    #            }
    #    with open(os.path.abspath(os.curdir)+ "/example_data/LABORATORY/{}.json".format(self.name),"w") as tofile:
...
def main():
    ...
    """
    test = PyExafsControl(facility_type='SYNCHROTRON', files=["/home/sepa/Downloads/someData/binned_data.txt"], take_first=False)
